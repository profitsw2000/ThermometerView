...
class SensorInfoFragment : BottomSheetDialogFragment()  {
    ...
    private fun initViews() = with(binding) {
        ...
        editHideTextView.setOnClickListener {
            if (updateSensorLetterGroup.visibility == View.VISIBLE) {
                ...
                editHideTextView.text = getString(ru.profitsw2000.core.R.string.show_edit_sensor_letter_form_text)
            } else {
                updateSensorLetterGroup.visibility = View.VISIBLE
                editHideTextView.text = getString(ru.profitsw2000.core.R.string.hide_edit_sensor_letter_form_text)
            }
        }
    }
    ...
    private fun loadSensorInfo() {
        if (sensorIndex > 0) sensorInfoViewModel.startSensorInfoFlow(sensorIndex)
        else setErrorText()
    }
